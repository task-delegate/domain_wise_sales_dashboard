...
const Sidebar: React.FC<SidebarProps> = ({ domains, activeDomain, setActiveDomain, onLogout, setCurrentView, activeView, openDeleteModal }) => {
  return (
    <aside className="w-full md:w-72 bg-slate-950 border-r border-slate-800 flex flex-col shadow-2xl h-auto md:h-screen overflow-y-auto">
      <div className="p-4 md:p-8 border-b border-slate-800 flex flex-col items-center gap-2 md:gap-4">
        <div className="relative group">
          ...
          <img 
            ...
            alt="GINZA Logo" 
            className="relative h-10 md:h-14 w-auto rounded-lg shadow-xl"
          />
        </div>
      ...
      <nav className="flex-1 px-2 md:px-4 py-4 md:py-6 space-y-1 md:space-y-1.5 overflow-y-auto custom-scrollbar">
        <p className="px-3 pb-2 md:pb-3 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-widest">Dashboards</p>
        {domains.map(domain => {
          ...
          return (
            <a
              ...
              className={`group flex items-center px-2 md:px-3 py-2 md:py-2.5 text-xs md:text-sm font-semibold rounded-xl transition-all duration-200 relative ${isActive ? 'bg-blue-600/10 text-blue-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}
            >
              {isActive && <div className="absolute left-0 w-1 h-6 bg-blue-500 rounded-r-full" />}
              <div className="flex-shrink-0 flex items-center justify-center w-7 md:w-8 h-7 md:h-8 rounded-lg transition-transform duration-200 group-hover:scale-110">
                  {domainLogos[domain] || <div className="w-7 md:w-8 h-7 md:h-8" />}
              </div>
              <span className="ml-2 md:ml-3 tracking-wide hidden sm:inline">{domain}</span>
            </a>
          );
        })}

        <p className="px-3 pt-6 md:pt-8 pb-2 md:pb-3 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-widest">Strategy Tools</p>
        <a
          href="#"
          onClick={(e) => { e.preventDefault(); setCurrentView('PPT'); }}
          className={`group flex items-center px-2 md:px-3 py-2 md:py-2.5 text-xs md:text-sm font-semibold rounded-xl transition-all duration-200 relative ${activeView === 'PPT' ? 'bg-blue-600/10 text-blue-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}
        >
          {activeView === 'PPT' && <div className="absolute left-0 w-1 h-6 bg-blue-500 rounded-r-full" />}
          <div className="flex-shrink-0 w-7 md:w-8 h-7 md:h-8 flex items-center justify-center">
            <PresentationIcon />
          </div>
          <span className="ml-2 md:ml-3 tracking-wide hidden sm:inline">AI Presentations</span>
        </a>
      </nav>

      <div className="p-2 md:p-4 border-t border-slate-800 space-y-1 flex flex-col md:flex-col gap-1">
        <button
          onClick={openDeleteModal}
          className="w-full flex items-center px-2 md:px-3 py-2 md:py-2.5 text-xs md:text-sm font-semibold rounded-xl text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
        >
          <DeleteIcon /> <span className="ml-2 md:ml-3 hidden sm:inline">Purge Data</span>
        </button>
        <button
          onClick={onLogout}
          className="w-full flex items-center px-2 md:px-3 py-2 md:py-2.5 text-xs md:text-sm font-semibold rounded-xl text-slate-400 hover:bg-slate-900 hover:text-white transition-all duration-200"
        >
          <LogOutIcon /> <span className="ml-2 md:ml-3 hidden sm:inline">End Session</span>
        </button>
      </div>
    </aside>
  );
